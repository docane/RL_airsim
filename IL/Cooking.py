...
def generateDataMapAirSim(folders):
    ...
    for folder in folders:
        ...
        current_df = pd.read_csv(os.path.join(folder, 'airsim_rec.txt'), sep='\t', encoding='cp949')
        ...
        for i in range(1, current_df.shape[0] - 1):
            ...
            norm_steering = [
                (float(current_df.iloc[i - 1][['Steering']]) + 1) / 2.0]  # Normalize steering: between 0 and 1
            norm_throttle = [float(current_df.iloc[i - 1][['Throttle']])]
            norm_speed = [
                float(current_df.iloc[i - 1][['Speed']]) / MAX_SPEED]  # Normalize speed: between 0 and 1
            ...
            image_filepath = os.path.join(os.path.join(folder, 'images'),
                                          current_df.iloc[i]['ImageFile']).replace('\\', '/')
...
def saveH5pyData(data_mappings, target_file_path, chunk_size):
    ...
    with h5py.File(target_file_path, 'w') as f:
        ...
        dset_previous_state = f.create_dataset('previous_state', shape=previous_state_chunk.shape,
