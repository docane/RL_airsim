...
max_pos = 1000
min_pos = -1000
pts_0 = np.concatenate((pts[1835:], pts[:1086]))
pts_1 = np.concatenate((pts[975:], pts[:45]))
print(pts_0[len(pts_0) - 56])
print(pts_1[len(pts_1) - 35])
print(pts_1[len(pts_1) - 45])
...
temp_0 = [0]
for i in range(1000):
    for j in range(temp_0[-1], len(pts_0)):
        if j > 1885:
            j = j % 1885
...
temp_1 = [0]
for i in range(1000):
    for j in range(temp_1[-1], len(pts_1)):
