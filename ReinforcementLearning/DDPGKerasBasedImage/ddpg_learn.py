...
class Actor(Model):
    ...
    def call(self, state_image):
        state = state_image[0]
        image = state_image[1]

        image_x = self.feature_extractor(image)
        x = concatenate([state, image_x])
        x = self.dense1(x)
        x = self.dense2(x)
        steering = self.steering(x)
        throttle = self.throttle(x)
        action = concatenate([steering, throttle])
...
class Critic(Model):
    ...
    def call(self, state_action_image):
        state = state_action_image[0]
        action = state_action_image[1]
        image = state_action_image[2]
...
class DDPGagent(object):
    def __init__(self, env):
        ...
        self.image_dim = env.observation_space['image'].shape
        ...
        image_in = Input((self.image_dim[0], self.image_dim[1], self.image_dim[2],))
        self.actor([state_in, image_in])
        self.target_actor([state_in, image_in])
        ...
        image_in = Input((self.image_dim[0], self.image_dim[1], self.image_dim[2],))
        self.critic([state_in, action_in, image_in])
        self.target_critic([state_in, action_in, image_in])
    ...
    def critic_learn(self, states, actions, td_targets):
        images = np.array([subarray['image'] for subarray in states])
        states = np.array([subarray['state'] for subarray in states])
        with tf.GradientTape() as tape:
            q = self.critic([states, actions, images], training=True)
    ...
    def actor_learn(self, states):
        ...
        states = np.array([subarray['state'] for subarray in states])
        with tf.GradientTape() as tape:
            actions = self.actor([states, images], training=True)
            critic_q = self.critic([states, actions, images])
    ...
    def get_action(self, obs):
        action = self.actor([tf.convert_to_tensor([obs['state']], dtype=tf.float32),
                             tf.convert_to_tensor([obs['image']], dtype=tf.float32)])
                    ...
                    target_qs = self.target_critic(
                        [tf.convert_to_tensor(np.array([subarray['state'] for subarray in next_states]),
                         ...
                         self.target_actor([
                             tf.convert_to_tensor(np.array([subarray['state'] for subarray in next_states]),
                                                  dtype=tf.float32),
                             tf.convert_to_tensor(np.array([subarray['image'] for subarray in next_states]),
                                                  dtype=tf.float32)]),
                         tf.convert_to_tensor(np.array([subarray['image'] for subarray in next_states]),
                                              dtype=tf.float32)])
